...
void demo_custom_loggers() {
  ...
  try {
    ...
    // 轮转文件日志器
    auto rotating_logger = spdlog::rotating_logger_mt("rotating", "logs/rotating.log", 1024 * 1024,
                                                      3);  // 1MB files, keep 3
...
void demo_performance() {
  ...
  std::cout << "  📊 记录 " << num_messages << " 条消息耗时: " << duration.count() << " ms\n";
  std::cout << "  📈 平均速度: " << (num_messages * 1000 / duration.count()) << " msg/sec\n";
}
...
void demo_structured_logging() {
  ...
  // 使用键值对格式
  logger->info("operation={} table={} duration_ms={} success={}", operation, table, duration_ms,
               success);

  // JSON风格的日志
  logger->info(
      R"({"event":"user_login","user_id":123,"ip":"192.168.1.1","timestamp":"2024-01-15T10:30:00Z"})");

  // 带上下文的错误日志
  std::string error_code = "AUTH_FAILED";
  std::string user_id = "user_456";
  logger->error("Authentication failed: code={} user_id={} attempts={}", error_code, user_id, 3);
}
