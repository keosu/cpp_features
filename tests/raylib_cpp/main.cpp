...
class ParticleSystem {
 ...
 public:
  ...
  size_t get_count() const { return particles.size(); }
};
...
class Paddle {
 public:
  ...
  Rectangle get_bounds() const { return {position.x, position.y, size.x, size.y}; }
};
...
class PongGame {
 ...
 public:
  ...
  void initialize_game() {
    // 创建游戏对象
    ball = std::make_unique<Ball>(Vector2{SCREEN_WIDTH / 2.0f, SCREEN_HEIGHT / 2.0f}, BALL_RADIUS);

    left_paddle = std::make_unique<Paddle>(Vector2{50, SCREEN_HEIGHT / 2.0f - PADDLE_HEIGHT / 2.0f},
                                           false  // 玩家控制
    );
  ...
  void draw_menu() {
    ...
    const char* instructions[] = {"SPACE - 开始游戏",   "↑/↓ 或 W/S - 移动挡板",
                                  "ESC - 暂停/退出",    "",
                                  "使用raylib游戏引擎", "集成现代C++特性"};

    int title_width = MeasureText(title, 60);
    int subtitle_width = MeasureText(subtitle, 30);

    DrawText(title, (SCREEN_WIDTH - title_width) / 2, SCREEN_HEIGHT / 2 - 100, 60,
             THEME_FOREGROUND);
    DrawText(subtitle, (SCREEN_WIDTH - subtitle_width) / 2, SCREEN_HEIGHT / 2 - 40, 30,
             THEME_ACCENT);

    for (size_t i = 0; i < sizeof(instructions) / sizeof(instructions[0]); ++i) {
      int text_width = MeasureText(instructions[i], 20);
      DrawText(instructions[i], (SCREEN_WIDTH - text_width) / 2, SCREEN_HEIGHT / 2 + 20 + i * 25,
               20, THEME_FOREGROUND);
    }
    ...
    if (((int)(game_time * 2)) % 2 == 0) {
      ...
      DrawText(prompt, (SCREEN_WIDTH - prompt_width) / 2, SCREEN_HEIGHT - 100, 24, THEME_SUCCESS);
    }
  }
  ...
  void draw_pause_overlay() {
    ...
    DrawText(pause_text, (SCREEN_WIDTH - pause_width) / 2, SCREEN_HEIGHT / 2 - 50, 48,
             THEME_FOREGROUND);
    DrawText(resume_text, (SCREEN_WIDTH - resume_width) / 2, SCREEN_HEIGHT / 2 + 20, 24,
             THEME_ACCENT);
  ...
  void draw_game_over() {
    ...
    DrawText(game_over_text, (SCREEN_WIDTH - game_over_width) / 2, SCREEN_HEIGHT / 2 - 80, 48,
             THEME_DANGER);
    DrawText(winner_message.c_str(), (SCREEN_WIDTH - winner_width) / 2, SCREEN_HEIGHT / 2 - 20, 36,
             THEME_SUCCESS);
    DrawText(restart_text, (SCREEN_WIDTH - restart_width) / 2, SCREEN_HEIGHT / 2 + 40, 24,
             THEME_ACCENT);

    // 最终分数
    std::string final_score =
        "最终比分: " + std::to_string(left_score) + " - " + std::to_string(right_score);
    int score_width = MeasureText(final_score.c_str(), 20);
    DrawText(final_score.c_str(), (SCREEN_WIDTH - score_width) / 2, SCREEN_HEIGHT / 2 + 80, 20,
             THEME_FOREGROUND);
  }
