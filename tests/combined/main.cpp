...
public:
    StudentManager() {
        ...
        try {
            ...
            auto file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>("student_system.log", true);
    ...
    void print_statistics() const {
        ...
        logger->info("统计信息 - 学生数: {}, 平均GPA: {:.2f}, 平均年龄: {:.1f}", 
                    students.size(), avg_gpa, avg_age);
    }
...
private:
    ...
    std::vector<std::string> course_list = {
        ...
        "English", "History", "Economics", "Psychology", "Art", "Music"
    };
...
public:
    ...
    std::vector<Student> generate_students(size_t count) {
        ...
        std::uniform_int_distribution<> course_dist(0, course_list.size() - 1);
        
        for (size_t i = 0; i < count; ++i) {
            std::string name = first_names[first_name_dist(gen)] + " " + 
...
void demo_search_and_filter() {
    ...
    for (double threshold : gpa_thresholds) {
        ...
        if (high_gpa_students.size() > 15) color = fmt::color::green;
        else if (high_gpa_students.size() > 5) color = fmt::color::yellow;
        else color = fmt::color::red;
